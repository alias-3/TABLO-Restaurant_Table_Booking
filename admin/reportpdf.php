<?php
require('../Dashboard/fpdf17/fpdf.php');
...
class PDF extends FPDF {
	function Header(){
		...
		$this->Cell(100,10,'       Bookings Report',0,1);
        ...
        $this->Cell(20,5,'Hotel',1,0,'',true);
        $this->Cell(20,5,'Name',1,0,'',true);
...
while($data=mysqli_fetch_array($result)){
    $stat = ' ';
    if($data['status']==0){$stat = "CANCELED";}else if($data['status']==2){$stat = "MISSED";}else if($data['status']==3){$stat = "COMPLETED";}
    $pdf->Cell(10,5,$data['bid'],'LR',0);
    $hotel1 = mysqli_query($conn,"SELECT hname FROM hotel WHERE hid = '{$data["hid"]}'"); $hot = mysqli_fetch_assoc($hotel1);
    $pdf->Cell(20,5,$hot["hname"],'LR',0);
    $pdf->Cell(20,5,$data['cusername'],'LR',0);
    $pdf->Cell(30,5,$data['cphno'],'LR',0);
